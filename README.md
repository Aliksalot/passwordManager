...
Това е юзър-фейсинг частта на приложението. В _gui_ се дефинира ```gui::Shell``` класът, който задава абстракция над ```std::cin``` и ```std::cout``` и менижира изпълнението на потребителски команди. Тук използвам функции от по-висок ред, което не е чисто ООП, но улеснява имплементацията на регистър с команди. ```gui::Shell``` съхранява в себе си масив от тип ```gui::Command```, където командата е проста структура
...
И една бележка към самото изпълнение - Без ```<function>``` хедъра не намерих начин да ламбда функциите да виждат променливи. Поради тази причина всичките изискват референция към себе си и към ```*this``` под формата на ```Shell& shell```. Алтернативата на това би била идея, подобна на тази, която съм използвал за ```CipherFactory```, но това би изисквало създаването на n малки класа, за n - броят на командите в приложението, което в момента се избягва. Сегашната имплементация позволява добавяне на команди чрез промяна на кода в единствен файл. 
...
която пак има същия ламбда проблем, за това изисква ```Shell&```
...
***Имплементация на шифри***

Hill Cipher по подразбиране работи само върху думи с дължина n за nxn ключ-матрица. В имплементацията на приложението той работи с думи с дължина kn за nxn ключ-матрица и k цяло число, което е направено като думата се разделя на части с дължина n, като всяка от тях се криптира/декриптира поотделно. В идеи за развитие на приложението е да се направи да работи за думи с произовлна дължина.
...
В момента ако потребителя е сбъркал една буква в дългата команда, която се опитва да напише, трябва да я пише наново - не може със "стрелка нагоре" да се върне, да поправи грешката си, и да продължи работа. Това би улеснило работата с приложението доста.

**6. Запазване на файл (Бъг)**

В момента се изисква от потребителя парола всеки път като иска да запази файла, което не е лошо. Проблемът е, че няма как да знаем дали потребителя използва същата парола за запазване на файл, която е използвал и за създаване. Съответно ако напише ```passwrod``` вместо ```password``` няма да изникне проблем веднага, но следващия път като се опита да си отвори файла, ще се чуди защо ```password``` не става. Решение на този проблем би било да се пазим някакъв хеш на паролата, с която потребителя е отворил файла. Този хеш може спокойно да се пази в рам паметта, докато се използва приложението, тъй като не може много да се помогне за разбиване на шифъра. Но ни дава възможност да разберем дали паролата за запазване от потребителя съвпада с паролата за отваряне на файл.
